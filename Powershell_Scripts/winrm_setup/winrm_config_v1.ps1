...
function New-WinRMHttpsListener {
    <#
    .SYNOPSIS
        Creates WinRM HTTPS listener using the provided certificate
    #>
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$CertInfo
    ...
    try {
        Write-Log -Message "Checking for existing HTTPS listeners..." -Level INFO
        
        # Remove existing HTTPS listener if present
        $ExistingListeners = Get-ChildItem -Path WSMan:\localhost\Listener | 
            Where-Object { $_.Keys -contains "Transport=HTTPS" }
        
        if ($ExistingListeners) {
            Write-Log -Message "Removing existing HTTPS listener(s)..." -Level WARNING
            $ExistingListeners | ForEach-Object {
                Remove-Item -Path $_.PSPath -Recurse -Force
            }
        }
        
        Write-Log -Message "Creating new HTTPS listener..." -Level INFO
        ...
        Write-Log -Message "Port: 5986" -Level INFO
