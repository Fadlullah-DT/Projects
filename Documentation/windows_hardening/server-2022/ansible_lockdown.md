...
- [Windows Server 2022 Hardening with Ansible Lockdown](#windows-server-2022-hardening-with-ansible-lockdown)
  ...
  - [Phase 3: Applying the CIS Benchmark](#phase-3-applying-the-cis-benchmark)
    ...
    - [2. Playbook (`windows2022_hardening.yml`)](#2-playbook-windows2022_hardeningyml)
    - [3. Execute the playbook](#3-execute-the-playbook)
    - [4. Run verification/audit mode](#4-run-verificationaudit-mode)
...
- A **Windows 11 workstation** acts as the **Ansible control node**.
...
```bash
ansible-galaxy install ansible-lockdown.windows_2022_cis
```

### 2. Playbook (`windows2022_hardening.yml`)

```yaml
- name: Apply CIS Level 1 Benchmark to Windows Server 2022
  hosts: windows_servers
  gather_facts: yes
  roles:
    - role: ansible-lockdown.windows_2022_cis
      vars:
        win22cis_level: "level1-domaincontroller"
        win22cis_create_domain: false
        win22cis_copy_policy_definitions: true
```

### 3. Execute the playbook

```bash
ansible-playbook -i inventory.ini windows2022_hardening.yml
```

### 4. Run verification/audit mode

```bash
ansible-playbook -i inventory.ini windows2022_hardening.yml --check
```
